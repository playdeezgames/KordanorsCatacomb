Module Program
    ...
    Sub Main(args As String())
        ...
        Using host As New Host(Of Hue, Command, Sfx)(
            "Larrikin of SPLORR!! ][ - Kordanor's Catacomb",
            gameController,
    ...
    Private Sub SaveConfig(windowSize As (Integer, Integer), volume As Single)
        File.WriteAllText(ConfigFileName, JsonSerializer.Serialize(New KCConfig With {.SfxVolume = volume, .WindowHeight = windowSize.Item2, .WindowWidth = windowSize.Item1}))
    End Sub
    Const DefaultWindowWidth = 1280
    Const DefaultWindowHeight = 720
    Const DefaultSfxVolume = 1.0F
    Private Function LoadConfig() As KCConfig
        Try
            Return JsonSerializer.Deserialize(Of KCConfig)(File.ReadAllText(ConfigFileName))
        Catch ex As Exception
            Return New KCConfig With
            {
